...
# Task 24.2 — Gerado por `rails generate simple_form:install --bootstrap -e erb`
# (Sprint 24, RF02/D2). Mesmo layout em dois arquivos do app de referência
# `basic8`: `simple_form.rb` (base default) + `simple_form_bootstrap.rb`
# (wrappers/componentes Bootstrap 5).
#
# POR QUE a precedência funciona: o Rails carrega os initializers em ordem
# alfabética, então `simple_form.rb` é executado primeiro e
# `simple_form_bootstrap.rb` depois — o segundo `SimpleForm.setup` sobrescreve
# `default_wrapper` para `:vertical_form` e define `wrapper_mappings`, que é o
# comportamento efetivo em runtime. Por isso a ordem dos arquivos não deve ser
# trocada nem os dois blocos podem ser fundidos numa ordem diferente.
#
# O template da simple_form 5.4.1 com `--bootstrap` já é 100% Bootstrap 5
# (classes form-control/form-select/invalid-feedback — sem legacy b3/b4).
